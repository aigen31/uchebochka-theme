...
try{
    $user_wrapper = new User_Wrapper($user_login);
    $user_data = $user_wrapper->get_user_data();
    $user_awatar = get_avatar_url($user_data->user_email, ['size' => 100]);
}catch(\Exception $e){
    $user_wrapper = false;
...
<div class="section-materials__materials add-material">
    ...
    <div class="profile">
        <div class="row">
            <div class="col-lg-4 col-md-5 col-12">
                <div class="ava">
                    <img src="<?=$user_awatar?>" alt="<?=$user_data->first_name?> <?=$user_data->last_name?>">
                </div>
            </div>
            <div class="col-lg-8 col-md-7 col-12">
                ...
                <div class="social">
                    <?if($user_data->vkprofile):?>
                        <a href="<?=$user_data->vkprofile?>" target="_blank"><img src="<?php echo get_template_directory_uri(); ?>/img/vk2.svg" alt="vk"></a>
                    <?endif;?>
                     <?if($user_data->tgprofile):?>
                        <a href="<?=$user_data->tgprofile?>" target="_blank"><img src="<?php echo get_template_directory_uri(); ?>/img/tg2.svg" alt="tg"></a>
                    <?endif;?>
                </div>
    ...
    <div class="info-user">
        <div class="item mat">
            <div class="num"><?=$user_wrapper->get_materials_count()?></div>
        ...
        <div class="item date">
            Продавец
            на платформе
            с <?=(new DateTime($user_data->user_registered))->format('d.m.Y');?>
        </div>
    </div>

    <?
    $materials = $user_wrapper->get_materials();
    if($materials):?>
    <div class="materials">
        <h2>Опубликованные материалы</h2>
        <?foreach($materials as $post){
            $post->author_avatar = $user_awatar;
            setup_postdata($post);
