...
<style>
  .robototehnika-catalog { font-family: "Wix Madefor Display", sans-serif; }
  ...
  .filter-btn:hover { background: transparent; }
          ...
          <div id="aiChatBlock" class="hidden">
            ...
            <div id="aiInputBlock">
              ...
              <textarea
                ...
                rows="3"
                class="w-full resize-none outline-none text-sm leading-relaxed max-h-[140px] overflow-y-auto border border-[#E5E7EB] rounded-lg p-3"
              ></textarea>
...
(function() {
  /*  FILTERS  */
  const filters = { for: null, age: null };
  ...
  function sendMessage() {
    ...
    .then(data => {
      ...
      // Check if we have suggestions with IDs
      // Check if PDF was generated
      if (data.pdf_url) {
        addMessage(reply, false);
        addPdfDownloadButton(data.pdf_url, data.pdf_title || 'Скачать материал');
      } else if (data.show_subscription_cta) {
        // AI couldn't find materials, show subscription CTA
        addMessage(reply, false);
